...
export default function SessionRunner({ routineId, showCheckInButton, onOpenCheckIn }: SessionRunnerProps) {
  ...
  useEffect(() => {
    ...
    if (routineId === "personal") {
      const s = localStorage.getItem(PERSONAL_ROUTINE_KEY);
    ...
    try {
      const stepRecords: StepRecord[] = steps.map((s, i) => {
        ...
        return {
          ...
          rating: p?.status === "rated" ? (p as any).rating : null,
          note: p?.note ?? null,
        };
      });
  ...
  if (!loaded || steps.length === 0) {
    return (
      <div className="bg-white rounded-3xl p-6 border border-[rgba(44,95,63,0.08)] shadow-sm text-center">
        ...
        <a href="/warmups/routine" className="inline-flex h-9 items-center px-5 rounded-full bg-[#2C5F3F] text-white text-[12px] font-semibold">
          Edit routine →
        </a>
      </div>
    );
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="space-y-2 mb-5">
          {steps.map((s, i) => {
            const p = progress[i];
            return (
              <div key={s.id} className="flex items-center justify-between rounded-xl bg-[#F9F8F5] px-4 py-3 border border-[rgba(44,95,63,0.06)]">
                <div className="min-w-0">
                  <p className="text-[13px] font-semibold text-[#1C2B22] truncate">
                    <span className="inline-flex w-5 h-5 rounded-full bg-[#EAF0EB] text-[10px] font-bold text-[#2C5F3F] items-center justify-center mr-2">{i + 1}</span>
                    {s.title}
                  </p>
                  {p?.note && <p className="text-[11px] text-[#8FA896] italic mt-0.5 pl-7">"{p.note}"</p>}
                </div>
                <span className={`text-[11px] font-semibold ml-3 shrink-0 ${
                  p?.status === "rated" ? "text-[#2C5F3F]" : p?.status === "skipped" ? "text-[#B5C4B9]" : "text-[#D5E0D8]"
                }`}>{statusLabel(p)}</span>
              </div>
            );
          })}
        ...
        <div className="mb-4">
          <p className="text-[12px] font-semibold text-[#1C2B22] mb-2">
            Final note <span className="text-[#8FA896] font-normal">(optional)</span>
          </p>
          <textarea
    ...
    <div className="bg-white rounded-3xl border border-[rgba(44,95,63,0.08)] shadow-sm overflow-hidden">
      <div className="px-5 pt-5 pb-4">
        <div className="flex items-center justify-between mb-2">
          <span className="text-[12px] font-medium text-[#8FA896]">Step {currentIdx + 1} / {total}</span>
          <span className="text-[12px] font-bold text-[#2C5F3F]">{pct}%</span>
        </div>
        <div className="h-1.5 w-full bg-[#EAF0EB] rounded-full overflow-hidden">
          <motion.div className="h-full bg-[#2C5F3F] rounded-full"
            animate={{ width: `${pct}%` }}
      ...
      <AnimatePresence mode="wait">
        <motion.div key={step.id}
          ...
          <h2 className="text-[20px] text-[#1C2B22] mb-1 leading-snug" style={{ fontFamily: "'Playfair Display', serif" }}>
            {step.title}
          </h2>
          <p className="text-[12px] text-[#8FA896] mb-4 leading-relaxed">{step.description}</p>
          ...
          <div className="flex items-center justify-between">
            <motion.button whileTap={{ scale: 0.97 }} onClick={goPrev} disabled={currentIdx === 0}
              className="h-10 px-5 rounded-full border border-[rgba(44,95,63,0.2)] text-[13px] font-semibold text-[#5A7A65] disabled:opacity-30 hover:bg-[#F5F2EC] transition-colors">
              Back
            </motion.button>
            {currentIdx === total - 1 && isDone ? (
              ...
              <motion.button whileTap={{ scale: 0.97 }} onClick={goNext} disabled={!isDone}
                className="h-10 px-6 rounded-full bg-[#2C5F3F] text-white text-[13px] font-semibold disabled:opacity-30 active:scale-95">
                Next →
              </motion.button>
            )}
          </div>
            ...
            return (
              <div key={s.id} className={`flex items-center justify-between rounded-xl px-3.5 py-2.5 border transition-all ${
                ...
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <span className={`text-[12px] font-medium truncate ${isCurrent ? "text-[#1C2B22]" : "text-[#5A7A65]"}`}>
                    {s.title}
                  </span>
                </div>
