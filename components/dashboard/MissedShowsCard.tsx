...
import { getShows, saveShows } from "@/lib/firebaseService";
...
const FEELING_GROUPS: { label: string; items: Feeling[] }[] = [
  { label: "Energy", items: ["Energetic", "Calm", "Fatigued", "Cool"] },
  { label: "Voice", items: ["Smooth", "Controlled", "Powerful", "Strained", "Tense"] },
  { label: "Mindset", items: ["Confident", "Nervous", "Focused", "Unfocused", "In the flow", "Taking risks"] },
];

const emptyShow = (): ShowEntry => ({
  name: "", time: "",
  ratings: { voice: 0, energy: 0, confidence: 0, range: 0 },
  feelings: [], note: "", done: false,
});
    ...
    <div className="bg-[#F9F8F5] rounded-2xl p-4 border border-[rgba(44,95,63,0.08)]">
      ...
      {show.feelings.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mb-1">
          {show.feelings.map((f) => (
            <span key={f} className="px-2.5 py-1 rounded-full text-[11px] font-medium bg-[#EAF0EB] text-[#2C5F3F]">{f}</span>
          ))}
        </div>
      )}
      {show.note && <p className="text-[12px] text-[#8FA896] italic mt-1">"{show.note}"</p>}
    </div>
  );
}
...
function ShowForm({ index, initial, onSave, onCancel }: {
  index: number; initial: ShowEntry;
  onSave: (s: ShowEntry) => void; onCancel: () => void;
}) {
  ...
  const toggleFeeling = (f: Feeling) =>
    setShow((s) => ({
      ...s,
      feelings: s.feelings.includes(f) ? s.feelings.filter((x) => x !== f) : [...s.feelings, f],
    }));

  const goNext = (from: number) => {
    if (from === 1 && !allRated) { setShowValidation(true); return; }
    setShowValidation(false); setStep(from + 1);
  };
    ...
    <div className="bg-[#F9F8F5] rounded-2xl p-4 border border-[rgba(44,95,63,0.1)]">
      ...
      {step === 0 && (
        <div>
          ...
          <input className="w-full bg-white border border-[rgba(44,95,63,0.15)] rounded-xl px-3.5 py-2.5 text-[13px] text-[#1C2B22] placeholder:text-[#B5C4B9] focus:outline-none focus:ring-2 focus:ring-[rgba(44,95,63,0.2)] mb-4"
            placeholder="e.g. Jazz Club Saturday..."
            value={show.name} onChange={(e) => setShow((s) => ({ ...s, name: e.target.value }))} />
      ...
      {step === 3 && (
        <div>
          <p className="text-[13px] font-semibold text-[#1C2B22] mb-1">Any notes?</p>
          <p className="text-[11px] text-[#8FA896] mb-3">Optional — but valuable over time.</p>
          <textarea className="w-full bg-white border border-[rgba(44,95,63,0.15)] rounded-xl px-3.5 py-2.5 text-[13px] text-[#1C2B22] placeholder:text-[#B5C4B9] focus:outline-none focus:ring-2 focus:ring-[rgba(44,95,63,0.2)] resize-none"
            rows={3} placeholder="e.g. Voice felt warm, high notes came easier..."
            value={show.note} onChange={(e) => setShow((s) => ({ ...s, note: e.target.value }))} />
          <div className="flex justify-between items-center mt-4">
...
// ── Main Card ─────────────────────────────────────────────────
export default function MissedShowsCard() {
  ...
  useEffect(() => {
    async function checkMissingDays() {
      ...
      for (let i = 1; i <= 7; i++) {
        const d = new Date();
        if (new Date().getHours() < 6) d.setDate(d.getDate() - 1);
        d.setDate(d.getDate() - i);
        const dateStr = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
        if (dateStr === today) continue;
        try {
          const existing = await getShows(dateStr);
          if (!existing || existing.shows.length === 0) {
            candidates.push(dateStr);
          }
        } catch { candidates.push(dateStr); }
      }

      setMissingDates(candidates);
      if (candidates.length > 0) setSelectedDate(candidates[0]);
      setLoading(false);
    }
    checkMissingDays();
  }, [savedDates]);
  ...
  const handleSelectDate = async (date: string) => {
    ...
    setShows([null]);
    setEditingIdx(null);
    // Load existing shows for that date if any
    try {
      const existing = await getShows(date);
      if (existing && existing.shows.length > 0) {
        const loaded = existing.shows.map((s) => ({ ...s, done: true } as ShowEntry));
        setShows(loaded.length > 0 ? [...loaded, null] : [null]);
      } else {
        setShows([null]);
      }
    } catch { setShows([null]); }
  ...
  const persistShows = async () => {
    ...
    const done = shows.filter((s): s is ShowEntry => s?.done === true);
  ...
  const activeMissing = missingDates.filter((d) => !savedDates.includes(d));

  if (loading) return null;
  if (activeMissing.length === 0) return null;

  const suggested = activeMissing[0];
  ...
  return (
    <div className="mx-4 bg-white rounded-3xl p-5 border border-[rgba(44,95,63,0.08)] shadow-sm">
      ...
      <div className="flex items-start gap-3 mb-4">
        <div className="w-9 h-9 rounded-2xl bg-amber-50 border border-amber-200 flex items-center justify-center shrink-0 text-lg">
          📋
        </div>
        <div className="min-w-0">
          <h2 className="text-[15px] font-semibold text-[#1C2B22]"
            style={{ fontFamily: "'Playfair Display', serif" }}>
            Did you have shows on {formatDateShort(suggested)}?
          </h2>
          <p className="text-[12px] text-[#8FA896] mt-0.5">
            You didn't log any shows that day. Add them now to keep your history complete.
          </p>
        </div>
      </div>
      ...
      {!open ? (
        <div className="space-y-3">
          <div className="flex gap-2">
            <button onClick={() => { handleSelectDate(suggested); setOpen(true); }}
              className="flex-1 h-10 rounded-full bg-[#2C5F3F] text-white text-[12px] font-semibold active:scale-[0.98] transition-transform">
              Yes, add shows →
            </button>
            <button onClick={() => setSavedDates((p) => [...p, suggested])}
              className="h-10 px-4 rounded-full border border-[rgba(44,95,63,0.15)] text-[12px] font-medium text-[#8FA896] hover:bg-[#F5F2EC] transition-colors">
              No shows
            </button>
          </div>

          {/* Other missing dates */}
          {activeMissing.length > 1 && (
            <div>
              <p className="text-[10px] font-bold tracking-widest uppercase text-[#B5C4B9] mb-2">
                Other days without shows
              </p>
              <div className="flex flex-wrap gap-2">
                {activeMissing.slice(1).map((date) => (
                  <button key={date} onClick={() => { handleSelectDate(date); setOpen(true); }}
                    className="h-8 px-3 rounded-full border border-dashed border-[rgba(44,95,63,0.2)] text-[11px] font-medium text-[#8FA896] hover:bg-[#F5F2EC] hover:text-[#2C5F3F] transition-colors active:scale-95">
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between mb-1">
            <p className="text-[12px] font-semibold text-[#1C2B22]">
              Shows for <span className="text-[#2C5F3F]">{selectedDate ? formatDateLabel(selectedDate) : ""}</span>
            </p>
            {activeMissing.length > 1 && (
              <select
                value={selectedDate ?? ""}
                onChange={(e) => handleSelectDate(e.target.value)}
                className="text-[11px] text-[#5A7A65] bg-[#F5F2EC] border border-[rgba(44,95,63,0.15)] rounded-lg px-2 py-1 focus:outline-none">
                {activeMissing.map((d) => (
          ...
          {shows[shows.length - 1]?.done && editingIdx === null && (
            <button type="button"
              onClick={() => { setShows((prev) => [...prev, null]); setEditingIdx(shows.length); }}
              className="w-full h-10 rounded-2xl border border-dashed border-[rgba(44,95,63,0.15)] text-[12px] text-[#B5C4B9] font-medium hover:bg-[#F5F2EC] transition-colors">
              + Add another show
            </button>
          )}
          ...
          <div className="flex gap-2 pt-1">
            <button onClick={() => { setOpen(false); setShows([null]); setEditingIdx(null); }}
              className="h-10 px-4 rounded-full border border-[rgba(44,95,63,0.15)] text-[12px] font-medium text-[#8FA896] hover:bg-[#F5F2EC]">
              Cancel
            </button>
            {anyDone && (
              <button onClick={persistShows} disabled={saving}
                className="flex-1 h-10 rounded-full bg-[#2C5F3F] text-white text-[12px] font-semibold active:scale-[0.98] disabled:opacity-60">
